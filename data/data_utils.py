...
def build_multiple_train_valid_test_datasets(args, train_valid_test_num_samples, use_shared_fs=True, data_impl="mmap", mmap_warmup=False):
    ...
    if weight_by_num_documents:
        ...
        #         token_cnt_file_path = "{}_loss_token_cnt.txt".format(dp)
        #         assert os.path.exists(token_cnt_file_path), f"token cnt file not found: {token_cnt_file_path}"
        #         token_cnt = 0
        #         with open(token_cnt_file_path) as fd:
        #             token_cnt = int(fd.readline().strip())
        #         num_tokens.append(token_cnt)
        #     print_rank_0(f"> token(loss mask = 1) count: {num_tokens}")
        #     ds_weights = ds_fn(num_tokens)
        #     train_ds_weights, valid_ds_weights, test_ds_weights = ds_weights, ds_weights, ds_weights
...
def load_dataset_from_bin(args):
    ...
    test_iters = args.valid_iters
    train_val_test_num_samples = [
        train_iters * args.total_train_batch_size,
        valid_iters * args.total_train_batch_size,
        test_iters * args.total_train_batch_size,
    ]
    ...
    args.do_train = train_ds is not None and args.train_iters > 0
    args.do_valid = valid_ds is not None and args.valid_iters > 0
    args.do_test = test_ds is not None and args.valid_iters > 0
